...
```{r}
write.csv(clinic, "/home1/chengpre/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/chengpre/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 2. What is the data type of $gender? chr
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)? 50 years old
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this. 45 years old
...
```{r}
vital_status_na_mask <- ifelse(clinic$vital_status == "Not Applicable", TRUE, FALSE)
```
...
```{r}
under30_over60_mask <-ifelse(clinic$age_at_diagnosis < 35, TRUE, ifelse(clinic$age_at_diagnosis > 60, TRUE, FALSE))
```
...
# 1. What does the y axis represent? The y-axis represents the rate of survival for the patients.

# 2. What does the x axis represent? The x-axis represents how much time has passed.

# 3. What age group appears to have the greatest rate of survival? The middle aged group has the greatest survival rate.

# 4. What age group appears to have the worst rate of survival? The young group has the worst survival rate.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients? It looks like it is about 30%.

# 6. What is the p-value? Is it statistically significant and what does that tell us about the relationship between age and breast cancer survival? The p-value is 0.2, which is greater than 0.05. This indicates that there is no statistically significant relationship between age and breast cancer survival.
